...
//
// Third-party console.* output (e.g. lsv-cli printing CL urls with api keys) must end up
// in the central logger, which is what applies secret masking and JSON formatting.
// ---------------------------------------------------------------------------
...
describe('redirectConsoleToLogger', () => {
  ...
  it('formats multiple arguments the way console does', () => {
    console.error('failed for %s:', 'vault', { attempt: 2 });

    expect(logger.error).toHaveBeenCalledWith('failed for vault: { attempt: 2 }');
  });
